...
# construct the kernel bank, a list of kernel we're going to apply
...
# load the input image and vonvert it to grayscale
...
for (kernelName, K) in kernelBank:
    ...
    cv2.imwrite("{} - convole".format(kernelName), convolveOutput)
    cv2.imshow("{} - opencv".format(kernelName), opencvOutput)
    cv2.imwrite("{} - opencv".format(kernelName), opencvOutput)
    cv2.waitKey(0)
